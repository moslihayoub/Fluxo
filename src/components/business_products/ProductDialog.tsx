...
import { Package, Laptop, X, CheckCircle2 } from 'lucide-react';
...
export default function ProductDialog({ product, onClose }: ProductDialogProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    defaultPrice_cents: product?.defaultPrice_cents?.toString() || '',
    categoryId: product?.categoryId || '',
    isActive: product !== undefined ? product.isActive : true,
    isFree: product?.isFree || false,
    discountRate: product?.discountRate?.toString() || '',
    resellerPrice_cents: product?.resellerPrice_cents?.toString() || ''
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const parsedPrice = parseFloat(formData.defaultPrice_cents);
    if (formData.defaultPrice_cents && isNaN(parsedPrice)) {
      toast.error('Prix par défaut invalide.');
      return;
    }
    ...
    if (product) {
      updateProduct(product.id, {
        ...
        type: formData.type,
        defaultPrice_cents: parsedPrice || 0,
        categoryId: formData.categoryId || undefined,
        isActive: formData.isActive,
        isFree: formData.isFree,
        discountRate: formData.discountRate ? parseFloat(formData.discountRate) : undefined,
        resellerPrice_cents: formData.resellerPrice_cents ? parseFloat(formData.resellerPrice_cents) : undefined
      });
      ...
      <div className="relative sm:fixed sm:inset-y-0 sm:right-0 z-10 bg-white dark:bg-zinc-900 w-full max-w-md sm:max-w-none sm:w-[50%] rounded-3xl sm:rounded-none sm:rounded-l-3xl shadow-xl overflow-hidden flex flex-col max-h-[90vh] sm:max-h-none sm:h-full animate-in fade-in sm:slide-in-from-right duration-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    value={formData.defaultPrice_cents}
                    onChange={(e) => setFormData({ ...formData, defaultPrice_cents: e.target.value })}
                    placeholder="0.00"
                ...
                <div>
                  ...
                  <Input
                    ...
                    min="0"
                    step="0.01"
                    value={formData.resellerPrice_cents}
                    onChange={(e) => setFormData({ ...formData, resellerPrice_cents: e.target.value })}
                    placeholder="0.00"
                    iconRight={<span className="text-zinc-400 text-sm font-medium pr-2">MAD</span>}
        ...
        <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 flex justify-end gap-3 bg-zinc-50 dark:bg-zinc-800/40">
          ...
          <button
            type="submit"
            form="product-form"
            className="px-6 py-2.5 bg-violet-600 dark:bg-violet-500 text-white rounded-xl font-bold hover:bg-violet-700 dark:hover:bg-violet-600 transition-colors shadow-sm flex items-center gap-2"
          >
            {product ? <><CheckCircle2 className="w-4 h-4" /> Enregistrer</> : <><Package className="w-4 h-4" /> Créer</>}
          </button>
        </div>
